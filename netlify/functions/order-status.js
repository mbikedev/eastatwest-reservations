...
// Optional overrides (used if you don't want a DB round-trip): { email, name, lang, when, total, code }.
...
// PATCH the status and return the updated row.
...
function buildStatusEmail({ status, name, lang, when, total, code }) {
  ...
  const copy = {
    en: {
      ...
      codeLabel: 'Code',
      foot: accepted
        ? 'See you soon at East at West.'
        : 'Please call us if you would like to arrange another time.',
    ...
    fr: {
      ...
      codeLabel: 'Code',
      foot: accepted
        ? 'À bientôt chez East at West.'
        : 'Appelez-nous si vous souhaitez convenir d’un autre horaire.',
    ...
    nl: {
      ...
      codeLabel: 'Code',
      foot: accepted
        ? 'Tot snel bij East at West.'
        : 'Bel ons gerust om een ander moment af te spreken.',
...
// Compose a human "when" label from the stored delivery_date + delivery_time.
...
exports.handler = async (event) => {
  ...
  try {
    ...
    // Normalise the status — accept a few synonyms from the admin UI.
    const norm = String(status || '').toLowerCase();
    const resolved = (norm === 'confirmed' || norm === 'confirm' || norm === 'accepted' || norm === 'accept')
      ? 'confirmed'
      : (norm === 'rejected' || norm === 'reject' || norm === 'cancelled' || norm === 'canceled' || norm === 'declined')
        ? 'rejected'
        : null;
    ...
    if (orderId) {
      try {
        ...
        // If the caller supplied contact details we can still email; otherwise fail.
        if (!body.email) {
    ...
    if (email) {
      ...
      const subject = (subjectMap[resolved][lang] || subjectMap[resolved].en);
      await transporter.sendMail({
        from: `"East at West" <${process.env.SMTP_FROM_EMAIL}>`,
        to: email,
        subject,
        html: buildStatusEmail({ status: resolved, name, lang, when, total, code }),
      });
      emailed = true;
    }
